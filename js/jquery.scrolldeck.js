			...
			for (i=0; i<$('.animate-build').length; i++) {
				...
				switch (anim.attr('data-animation')) {
					...
					case 'fade-in':
						...
						break;
					case 'showInOut':
						var delay = (anim.attr('data-build') - 1) * 400 ;
						scrolldeck.controller.animate(anim, { delay: delay, duration: 400, property: 'opacity', start: 0, end: 1, pin: true, easing:'blink' });
						//scrolldeck.controller.animate(anim, { delay: delay + 400, duration: 0, property: 'opacity', start: 1, end: 0, pin: true });
						break;
					default:
						scrolldeck.controller.animate(anim, { delay: (anim.attr('data-build')-1)*400, duration: 400, property:'opacity', start:0, pin:true });
				}
